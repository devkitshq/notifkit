...
describe("EnricherWorker", () => {
  ...
  it("dispatches directly to outboundProducers when configured (Approach A pipeline collapse)", async () => {
    const mockOutboundProducers = {
      normal: {
        publishBatch: vi.fn().mockResolvedValue({ messageIds: ["out-1"], eventIds: ["evt-out-1"] }),
      },
    };

    const fastWorker = new EnricherWorker({
      consumer: mockConsumer as any,
      pendingScanner: {} as any,
      logger: mockLogger as any,
      maxRetriesBeforeDlq: 5,
      ...
      contactRepo: {
        findActiveByUserIds: vi
          ...
          .mockResolvedValue(
            new Map([
              ["usr-1", [{ channel: "email", target: "alice@example.com", enabled: true }]],
            ]),
          ),
      } as any,
      templateCache: {
    ...
    const msg = {
      ...
      event: {
        ...
        type: "notification.requested",
        timestamp: new Date().toISOString(),
        metadata: { traceId: "trace-collapse" },
        payload: {
          projectId: "123e4567-e89b-12d3-a456-426614174000",
          target: { type: "user", userId: "usr-1" },
          channels: ["email"],
    ...
    await fastWorker.process(msg as any);

    // Verify it bypassed intermediate mockProducers and published directly to mockOutboundProducers
    expect(mockProducers.normal.publishBatch).not.toHaveBeenCalled();
    expect(mockOutboundProducers.normal.publishBatch).toHaveBeenCalledTimes(1);

    const publishedBatch = (mockOutboundProducers.normal.publishBatch as any).mock.calls[0][0];
    expect(publishedBatch).toHaveLength(1);
    const dispatched = publishedBatch[0];
    expect(dispatched.type).toBe("notification.dispatched");
    expect(dispatched.payload.channel).toBe("email");
    expect(dispatched.payload.destination).toBe("alice@example.com");
    expect(dispatched.payload.renderedContent.content.subject).toBe("Welcome Alice");
  });
});
